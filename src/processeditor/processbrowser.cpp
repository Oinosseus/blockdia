...
ProcessBrowser::ProcessBrowser(QWidget *parent) : QWidget(parent)
{
    ...
    // load base path
    QSettings s;
    this->viewFsTree->setRootIndex(this->modelFs->index(s.value("ProcessBrowser/BasePath").toString()));
}

QString ProcessBrowser::currentRootPath()
{
...
void ProcessBrowser::slotDirBrowse()
{
    ...
    // save path
    QSettings s;
    s.setValue("ProcessBrowser/BasePath", dirPath);
}

void ProcessBrowser::slotItemClicked(QModelIndex idx)
{
